...
const ARTICLE_METADATA_TYPES = new Set(['article', 'newsarticle', 'blogposting']);
...
export function detectReadableArticle(doc, paragraphs, options = {}) {
  ...
  for (const root of candidates) {
    ...
    if (!best || candidate.confidence > best.confidence || (
      candidate.confidence === best.confidence && candidate.proseWords > best.proseWords
    )) {
      best = candidate;
    }
  }

  const isReadableArticle = best.confidence >= 0.65
    && best.proseWords >= 300
    && best.longProseBlockCount >= 3
    && best.linkDensity < 0.50;
...
function calculateConfidence({ metadataFound, root, proseWords, longProseBlockCount, linkDensity }) {
  let confidence = 0;

  if (metadataFound) confidence += 0.25;
  if (root.tagName === 'ARTICLE') confidence += 0.20;
  if (root.tagName === 'MAIN') confidence += 0.10;
  if (proseWords >= 300) confidence += 0.20;
  if (proseWords >= 600) confidence += 0.20;
...
function visitVisibleText(node, isVisible, onText) {
  ...
  node.childNodes.forEach(child => visitVisibleText(child, isVisible, onText));
}
...
function hasSkippedOrHiddenSelf(element) {
  return SKIP_TAGS.has(element.tagName) || element.getAttribute('aria-hidden') === 'true';
}

function hasArticleMetadata(doc) {
  return hasOpenGraphArticleType(doc)
...
function hasJsonLdArticleType(doc) {
  return Array.from(doc.querySelectorAll('script[type="application/ld+json"]')).some(script => {
    try {
...
function hasItemtypeArticleType(doc) {
  return Array.from(doc.querySelectorAll('[itemtype]')).some(element => isArticleType(element.getAttribute('itemtype')));
}

function isArticleType(value) {
  const normalized = String(value || '').toLowerCase();
  return Array.from(ARTICLE_METADATA_TYPES).some(type => normalized.includes(type));
}
